...
export function runSjf(processes: SchedulingProcess[]): SimulationResult {
  ...
  while (pendingProcesses.length > 0) {
    ...
    const nextProcess = availableProcesses.reduce((shortest, process) => {
      if (process.burstTime !== shortest.burstTime) {
        return process.burstTime < shortest.burstTime ? process : shortest;
      }

      if (process.arrivalTime !== shortest.arrivalTime) {
        return process.arrivalTime < shortest.arrivalTime ? process : shortest;
      }

      return process.index < shortest.index ? process : shortest;
    });
